...
namespace HighElixir.StateMachines.Extension
{
    ...
    public static class RegisterExt
    {
            ...
            
        {
            ...
            foreach ((TEvt evt, TState to, Action<StateMachine<TCont, TEvt, TState>.TransitionResult> action, Func<StateMachine<TCont, TEvt, TState>.TransitionResult, bool> predicate) in transes)
                dis.Add(s.RegisterTransition(from, evt, to, action, predicate));

            if (dis.Count == 0) return Disposable.Empty;

            var d = dis[0];
            dis.Remove(d);
            return d.JoinTo(dis.ToArray());
        }
        ...
        {
            foreach (var (evt, to) in transes)
                s.RegisterTransition(from, evt, to);
        }
        ...
        {
            foreach (var (evt, toState) in transes)
                s.RegisterAnyTransition(evt, toState);
        }
        ...
        {
            var dis = new List<IDisposable>();
            foreach (var (evt, to, action, predicate) in transes)
                dis.Add(s.RegisterAnyTransition(evt, to, action, predicate));
