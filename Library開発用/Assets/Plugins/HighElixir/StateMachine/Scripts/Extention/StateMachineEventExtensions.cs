...
namespace HighElixir.StateMachines.Extension
{
    ...
    {
        public static void SendToForget<TCont, TEvt, TState>(
            this StateMachine<TCont, TEvt, TState> stateMachine,
            TEvt evt, CancellationToken token = default)
            => _ = stateMachine.Send(evt, token);
        public static void AwakeToForget<TCont, TEvt, TState>(
            this StateMachine<TCont, TEvt, TState> stateMachine, TState state, CancellationToken token = default)
            => _ = stateMachine.Awake(state, token);
