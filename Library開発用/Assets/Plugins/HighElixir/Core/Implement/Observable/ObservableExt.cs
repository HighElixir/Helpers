...
namespace HighElixir.Implements.Observables
{
    ...
    public static class ObservableExt
    {
        ...
        /// </summary>
        public static IDisposable JoinTo(this IDisposable source, params IDisposable[] disposables)
        {
        ...
        /// 複数のIDisposableをまとめて破棄できるようにする
        /// </summary>
        public static IDisposable CompositeFrom(ref IDisposable source, params IDisposable[] disposables)
        {
            if (source is UniteDisposable unite)
                return unite.Add(disposables);
            else
            {
                var tmp = new UniteDisposable(disposables);
                tmp.JoinTo(source);
                source = tmp;
                return tmp;
            }
