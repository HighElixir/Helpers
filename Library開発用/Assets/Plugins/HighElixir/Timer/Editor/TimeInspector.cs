    ...
    public class TimeInspector : EditorWindow
    {
            ...
            ITimer timer = null)
        {
            ...
            using (var scope = new EditorGUILayout.HorizontalScope())
            {
                ...
                state.textColor = fontColor != default ? fontColor : GUI.skin.label.normal.textColor;
                var style = new GUIStyle(GUI.skin.label);
                style.normal = state;
                // Parent
                EditorGUILayout.LabelField(parent, style, GUILayout.Width(120));

                // Name
                EditorGUILayout.LabelField(name, style, GUILayout.Width(120));

                // CountType
                EditorGUILayout.LabelField(countType, style, GUILayout.Width(140));
                ...
                if (timer != null)
                {
                    ...
                    EditorGUILayout.LabelField(running, style, size);

                // 完了フラグ（✔）
                EditorGUILayout.LabelField(finished, style, GUILayout.Width(50));

                // option が空じゃなければ表示
                if (!string.IsNullOrEmpty(option))
                    EditorGUILayout.LabelField(option, style, GUILayout.Width(optionWidth));
            }
        }
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                //}
                if (GUILayout.Button("Color Editor", EditorStyles.toolbarButton, GUILayout.Width(100)))
                {
                    ColorEditorWindow.ShowWindow(_cache);
                }
